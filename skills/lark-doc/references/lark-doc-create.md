...
> 3. [`lark-doc-create-workflow.md`](style/lark-doc-create-workflow.md) — 创建文档的操作流程
>
...
- **禁止行内代码块**：不要在 Markdown 中使用单反引号行内代码；XML 的 `<code>` 只能放在 `<pre>` 内作为代码块内容。
- **文字样式**：可使用 `<b>`、`<u>`、`<em>`、`<span text-color>`、`<span background-color>` 等文字样式提升可读性。

## 参考

- [`lark-doc-create-workflow.md`](style/lark-doc-create-workflow.md) — 创建文档的操作流程
