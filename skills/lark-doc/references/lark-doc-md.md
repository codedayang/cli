...
> **禁止使用行内代码块 / inline code：** 写入 Markdown 内容时不要用单反引号包裹文本。需要展示代码片段时，使用 Markdown fenced code block，或改用 XML 的 `<pre><code>...</code></pre>`。
...
**不需要转义的场景：**
- 在 Markdown fenced code block 内，所有符号均为字面量，无需转义
...
如需使用加粗、下划线、颜色、背景色等文字样式，优先改用 XML；Markdown 原生能力不足时不要用行内代码替代强调样式。
